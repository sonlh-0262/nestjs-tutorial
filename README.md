...
| Uploads    | Multer (memory storage) -> local disk, polymorphic `attachments` table |
...
| PUT    | `/user`                      | yes      | Update the current user, optionally uploading an avatar |
...
curl -X PUT http://localhost:3000/user \
  ...
  -d '{"user":{"bio":"I work at statefarm"}}'

# Update it and upload an avatar in one multipart request.
curl -X PUT http://localhost:3000/user \
  -H "Authorization: Token $TOKEN" \
  -F 'user[bio]=I work at statefarm' \
  -F 'avatar=@picture.png'

# The response `image` is /attachments/<uuid>, and fetching it needs the token
curl -o avatar.png http://localhost:3000/attachments/<uuid> \
  -H "Authorization: Token $TOKEN"

# Profiles - readable anonymously, but `following` needs a token to be useful
curl http://localhost:3000/profiles/jake
curl http://localhost:3000/profiles/jake -H "Authorization: Token $TOKEN"
...
├── app.module.ts              # Wires Config, I18n, DB, Redis, Users, Auth, Profiles, Attachments
├── main.ts                    # Bootstrap
├── auth/
...
├── users/
│   ├── users.controller.ts    # GET /user, PUT /user
│   ├── users.service.ts       # All reads and writes of the users table
│   ├── follows.service.ts     # The user_follows edge table
│   ├── password.service.ts    # The one place passwords are hashed/verified
│   ├── users.constants.ts     # Field limits shared by register and update
│   ├── dto/                   # Public projection, `user` envelope, update DTO
...
│   ├── entities/attachment.entity.ts  # Polymorphic attachments table
│   └── storage/
│       ├── local-file-storage.ts   # Disk driver, guarded against traversal
│       ├── image-type.ts           # Magic-number format detection
│       ├── file-name.ts            # File-name sanitiser
│       └── multer.config.ts        # Memory storage + size limit
├── database/
